...
{
    if ( pRoot2 == nullptr )//说明子结构遍历结束,找到
    {
        return true;
    }

    if ( pRoot1 == nullptr )//子结构还没结束但是树就结束了
    {
        return false;
    }
...
{
    bool bResult = false;
    if ( pRoot1!=nullptr &&  pRoot2!=nullptr )//空指针
    {
        if ( Equal(pRoot1->m_dbValue,pRoot2->m_dbValue) )//比较两个结点值
        {
            bResult =  DoesTree1HaveTree2(pRoot1,pRoot2);
        }  
