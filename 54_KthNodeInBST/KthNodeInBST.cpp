...
//==================求第k个结点 循环 中序遍历二叉数
const BinaryTreeNode* KthNode_1(const BinaryTreeNode* pRoot, unsigned int& k)
{
    if (pRoot == nullptr)
        return nullptr;

    stack<const BinaryTreeNode*> s;
    const BinaryTreeNode*   t = pRoot; 
    
    while ( (t!=nullptr) || !s.empty() )//节点不空或栈不空
    {
        if ( t!=nullptr)
        {
            s.push(t);    
...
void Test(const char* testName, const BinaryTreeNode* pRoot, unsigned int k, bool isNull, int expected)
{
    ...
    const BinaryTreeNode* pTarget = KthNode_1(pRoot, k);
