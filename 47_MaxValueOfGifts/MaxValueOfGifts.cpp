...
int getMaxValue_solution1(const int* values, int rows, int cols)
{
    ...
    for (int i=0; i<rows; ++i)
    {
        for (int j=0; j<cols; ++j)
        {
            ...
            MaxValue[i][j] = std::max(up,left) + values[cols*i+j];
           
        }
        
    }

    int nMaxValue = MaxValue[rows-1][cols-1];
    for (int i=0; i<rows; ++i)
    {
        delete [] MaxValue[i];
        MaxValue[i] = nullptr;
...
int getMaxValue_solution2(const int* values, int rows, int cols)
{
    ...
    for (int i=0; i<rows; ++i)
    {
        for (int j=0; j<cols; ++j)
        {
            ...
            MaxValue[j] = std::max(up,left) + values[cols*i+j];
           
        }
        
    }
    
    int nMaxValue = MaxValue[cols-1];
    delete [] MaxValue;
    MaxValue = nullptr;
    return nMaxValue;
}
